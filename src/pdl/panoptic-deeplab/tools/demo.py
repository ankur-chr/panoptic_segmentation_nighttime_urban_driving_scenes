...
#   TEST.MODEL_FILE PATH_TO_MODEL_FILE
# ------------------------------------------------------------------------------
#
# Updated by Ankur Chrungoo
# File sorting added to maintain consistency in the results.
# Indexing updated in the logs
#
import argparse
import cv2
import os
...
def main():
    ...
    # Sort the input file list, so that it is easy to compare the results.
    print(input_list)
    input_list.sort()
            ...
            for i, fname in enumerate(input_list):
                ...
                logger.info('[{}/{}]\t'
                            ...
                            'Post-processing Time: {post_time.val:.3f}s ({post_time.avg:.3f}s)\t'.format(
                             i+1, len(input_list), net_time=net_time, post_time=post_time))
