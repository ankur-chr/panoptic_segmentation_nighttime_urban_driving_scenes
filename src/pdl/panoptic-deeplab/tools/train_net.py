# Code for training.
# Example command: python -m torch.distributed.launch --nproc_per_node=4 tools/train_net.py --cfg PATH_TO_CONFIG_FILE
#
# Modified by Ankur Chrungoo
# -------------------------------------------------------------------------------------------------------------
...
def main():
    ...
    # This will not be used in training due to single GPU availability
    if distributed:
    ...
    # Get the training data loader from the configuration
    ...
    start_iter = 0
    # Max number of iterations loaded from the configuration file.
    max_iter = config.TRAIN.MAX_ITER
    best_param_group_id = get_lr_group_id(optimizer)

    # This code is not used currently! Pretrained backbone is loaded resnet.py, for example!
    # initialize model
    ...
    if config.TRAIN.RESUME:
        ...
        if os.path.isfile(model_state_file):
            ...
            # Modified to allow refinement of an existing model with a fresh learning rate.
            # Code updated by Ankur Chrungoo
            # lr_scheduler.load_state_dict(checkpoint['lr_scheduler'])
            if config.TRAIN.REFINE:
                print("Refining existing model with fresh learning rate schedule")
            else:
                lr_scheduler.load_state_dict(checkpoint['lr_scheduler'])
