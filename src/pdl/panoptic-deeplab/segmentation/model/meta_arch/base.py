# ------------------------------------------------------------------------------
# Segmentation base model code (with backbone, decoder, etc)
...
# Class for Basic Segmentation Model
class BaseSegmentationModel(nn.Module):
    ...
    def forward(self, x, targets=None):
        ...
        # Run the features through the decoder
        pred = self.decoder(features)
        # Up-sample the predictions
        results = self._upsample_predictions(pred, input_shape)

        if targets is None:
            return results
        else:
            # return the loss (based on predictions and the targets)
